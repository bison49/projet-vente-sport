...
function UserSoldArticle() {
  const { user } = useUser();
  const id = user.id;
  const [url, setUrl] = useState(`api/articles?page=1&seller.id=${id}&isSold=true`);
  const [articles, setArticles] = useState<any[]>([]);
  const today = new Date();
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [noListMessage, setNoListMessage] = useState<string>(
    "Vous n'avez pas encore vendu de biens sur notre site.",
  );
  ...
  function onChange(e: { target: { value: any } }) {
    setUrl(`api/articles?page=1&seller.id=${id}&isSold=true${e.target.value}`);
    setNoListMessage("Vous n'avez pas vendu de biens durant cette période.");
  }
